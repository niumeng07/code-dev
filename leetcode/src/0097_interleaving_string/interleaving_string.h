...
public:
    ...
    bool isInterleave(string s1, string s2, string s3) {
        ...
        if (char(s1[0]) == char(s3[0]) && char(s2[0]) != char(s3[0]))
            return isInterleave(s1.substr(1, s1.size() - 1), s2,
                                s3.substr(1, s3.size() - 1));
        if (char(s2[0]) == char(s3[0]) && char(s1[0]) != char(s3[0]))
            return isInterleave(s1, s2.substr(1, s2.size() - 1),
                                s3.substr(1, s3.size() - 1));
        if (char(s2[0]) == char(s3[0]) && char(s1[0]) == char(s3[0]))
            return isInterleave(s1.substr(1, s1.size() - 1), s2,
                                s3.substr(1, s3.size() - 1)) ||
                   isInterleave(s1, s2.substr(1, s2.size() - 1),
                                s3.substr(1, s3.size() - 1));
        return false;
    }
    /*
     ...
     * 迭代公式: f(i,j) = ({ f(i-1,j)==true } && s1[i-1]== s3[i+j-1]) || ({ f(i,
     * j-1)==true } && s2[j-1] == s3[i+j-1] ) 边界条件，i==0或j==0
     * 初始条件若i==0,且j==0,则f(i,j)=true
     * (s1的前0个字符和s2的前0个字符能组成s3的前0个字符）
     * */
