...
int repartition(vector<int> &input, int left, int right) {
    ...
    while (i < j) {
        if (high_num(input[i]) > high_num(input[middle]) &&
            high_num(input[j]) < high_num(input[middle])) {
            swap(input[i], input[j]);
            i++;
            j--;
        } else {
            if (high_num(input[i]) <= high_num(input[middle])) i++;
            if (high_num(input[j]) >= high_num(input[middle])) j--;
        }
    }
...
void qs_impl(vector<int> &input, int left, int right) {
    ...
    if (right > left + 1) {
        ...
        qs_impl(input, middle, right);
               ...
               high_num(input[right]) < high_num(input[left])) {
        swap(input[left], input[right]);
    }
}
void quick_sort(vector<int> &input) { qs_impl(input, 0, input.size() - 1); }
class LargestNumber {
public:
