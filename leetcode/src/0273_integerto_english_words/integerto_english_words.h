...
IntegertoEnglishWords::DigitArray IntegertoEnglishWords::intToDigitArray(
        int num, int &digitCount) {
    DigitArray result;
    ...
    while (i < result.size()) result[i++] = -1;

    return result;
}

bool IntegertoEnglishWords::convert(std::int8_t hundred, std::int8_t ten,
                                    std::int8_t unit, std::string &result) {
...
std::string IntegertoEnglishWords::numberToWords(int num) {
    ...
    switch (digitCount) {
        case 10:
            if (convert(0, 0, digits[9], result)) result.append(" Billion");
        ...
        case 1:
            ...
            if (result.empty() && !digits[0]) result = "Zero";
    }
