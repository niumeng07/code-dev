...
public:
    ...
    {
        if( tmp.size() == this->N - 1 )
        {
            if( this->sumTmp <= this->target ||( tis->sumTmp > this->target && nums[max_i] < 0))
            this->result.push_back(tmp);
            this->last.push_back(this->target - this->sumTmp);
            this->sumTmp -= tmp.back();
            tmp.pop_back();
        }
        else if(tmp.size() < this->N - 1 )
        {
